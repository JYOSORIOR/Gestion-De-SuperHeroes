...
def crearPelea(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error_message = f'El héroe con el ID {id_heroe} no existe.'
        else:
            try:
                villano = Villano.objects.get(pk=id_villano)
            except Villano.DoesNotExist:
                error_message = f'El villano con el ID {id_villano} no existe.'
            else:
    ...
    peleas = Pelea.objects.all()
    return render(request, 'gestionPeleas.html', {'error_message': error_message, 'pelea': peleas})
...
def crearPatrocinador(request):
    ...
    heroes = Heroe.objects.all()
    return render(request, 'gestionPatrocinador.html', {'error_message': error_message, 'heroes': heroes})
